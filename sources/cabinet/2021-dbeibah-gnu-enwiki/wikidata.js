...
module.exports = function () {
  ...
  return `SELECT DISTINCT ?item ?itemLabel ?position ?positionLabel ?cabinet
                 ?startDate ?endDate ?source (STRAFTER(STR(?ps), STR(wds:)) AS ?psid)
    WITH {
      SELECT DISTINCT ?item ?position ?cabinet ?startNode ?endNode ?ps
      WHERE {
          ...
          FILTER (!BOUND(?cabinet) || (?cabinet != wd:Q113638573)) # Not in the competing cabinet
